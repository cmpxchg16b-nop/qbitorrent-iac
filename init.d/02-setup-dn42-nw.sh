...
log "Creating veth pair: v-qb (ns=$ROUTER_PID) <-> v-dn42 (ns=$APP_NETNS)"
ip link add v-qb netns "$ROUTER_PID" type veth peer v-dn42 netns "$APP_NETNS"
...
nsenter --net="$APP_NETNS" ip route add 172.20.0.0/14 via "$DN42_GW_IP6_ADDR" dev v-dn42

log "Adding route 10.127.0.0/16 via $DN42_GW_IP6_ADDR (extended nexthop)"
nsenter --net="$APP_NETNS" ip route add 10.127.0.0/16 via "$DN42_GW_IP6_ADDR" dev v-dn42
...
nsenter -t "$ROUTER_PID" -n ip route add "$DN42_IP" dev v-qb
